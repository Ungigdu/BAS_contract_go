...
contract BAS_Manager is owned{
    ...
    function split(bytes memory data) internal pure returns (bytes4 f, bytes16 s, bytes32 b){
        assembly{
            f := mload(add(data,32))
            s := mload(add(data,36))
            b := mload(add(data,52))
    ...
    function remember(bytes32 hashKey,bytes4 f, bytes16 s, bytes32 b) internal{
        Record memory r = Record({IPv4:f,IPv6:s,BCAddress:b,EthAddress:msg.sender});
    ...
    function insert (string memory key, bytes memory data) internal{
        require(data.length==52,"bad format");
        bytes32 hashKey = hash(key);
        (bytes4 f, bytes16 s, bytes32 b) = split(data);
        require(checkAvailablity(hashKey, f, s, b),"not available");
        remember(hashKey,f,s,b);
    }
    ...
    function queryByHash(bytes32 hashKey) public view returns (bytes32,bytes4,bytes16,bytes32){
        Record memory r = DataRecords[hashKey];
        return (hashKey,r.IPv4,r.IPv6,r.BCAddress);
    }
    function queryByString(string memory key)public view returns (bytes32,bytes4,bytes16,bytes32){
        bytes32 hashKey = hash(key);
        return queryByHash(hashKey);
    }
    function queryByIPv4(bytes4 IPv4) public view returns (bytes32,bytes4,bytes16,bytes32){
        bytes32 hashKey = IPv4Mapping[IPv4];
        return queryByHash(hashKey);
    }
    function queryByIPv6(bytes16 IPv6) public view returns (bytes32,bytes4,bytes16,bytes32) {
        bytes32 hashKey = IPv6Mapping[IPv6];
        return queryByHash(hashKey);
    }
    function queryByBCAddress(bytes32 bc) public view returns (bytes32,bytes4,bytes16,bytes32) {
